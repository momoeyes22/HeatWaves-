...
variable = 'tmax' # give variable name (rain for rainfall, tmax or tmin for min or max temperature)
file_format = 'yearwise' # other option (None), which will assume deafult imd naming convention
imd.get_data(variable, start_yr, end_yr, fn_format='yearwise', file_dir='C:\data') # download IMD data: just change path as per your requirement
file_dir = 'C:\data' # this path should be same as mentioned in previous line
...
for yr in range(0,years_no):
    f = open("C:/Users/megha/OneDrive/Desktop/maxtemps"+str(start_yr+yr)+"_"+str(variable)+".csv",'w') # just change the path where you want to save csv file
